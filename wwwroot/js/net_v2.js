...
APIPlot.prototype.netPlotCytoscape = function (data_network_raw) {

    //var data_network = APIPlot.prototype.buildJSONtoPlot(data_network_temp);
    ...
    var number_of_genes = data_network_raw.length;
    if (number_of_genes >= 200) {
        //node_min = 0
        max_link_value = 0
        min_link_value = 1
        ...
        min_link_value = min_link_value * 2
    } else {
        min_link_value = 0
        max_link_value = 1
    }
    json_data = '{  "data" : { "shared_name":"TEST","name" : "TWST","SUID" : 178823,"__Annotations" : [],"selected" : true}, "elements" : {"nodes" : [';
    console.log(min_link_value)
    for (var i = 0; i < number_of_genes; i++) {
        json_data += '{ "data": {"id": "' +  i + '","label": "' + data_network_raw[i][0] + '" } }';
        //cy.add({
        //    data: {
        //        id: data_network_raw[i][0]
        //    }
        //});
    ...
    for (var i = 0; i < number_of_genes; i++) {
        ...
        local_id += i
        for (var j = 0; j < number_of_genes; j++) {
            if (start_storing && (data_network_raw[i][j] > min_link_value)) {
                local_id += j
                json_data += '{ "data": {"id": "' + local_id + '","source": "' + i + '","target": "' + j + '","weight": "' + data_network_raw[i][j] + '" } },';
                //cy.add({
                //    data: {
                //        id: i + j,
                //        source: data_network_raw[i][0],
                //        target: data_network_raw[j][0],
    ...
    json_data += ']}}';console.log(json_data)
...
APIPlot.prototype.netPlot2 = function (data_network_temp) {
    !(function () {
        "use strict"
        ...
        function main(data_network) {
            ...
            var data = {
                nodes: data_network.nodes.map(function (d) { return { label: d.id, r: d.score * 2 } }),
                links: data_network.links.map(function (d) { return { source: d.source, target: d.target } })
            }
        ...
        function setSize() {
            width = document.querySelector("#network_plot").clientWidth
            height = document.querySelector("#network_plot").clientHeight
...
////function dragstarted(d) {
////    if (!d3.event.active) APIPlot.prototype.simulation.alphaTarget(0.3).restart();
////    d.fx = d.x;
////    d.fy = d.y;
////}
...
////function dragended(d) {
////    if (!d3.event.active) APIPlot.prototype.simulation.alphaTarget(0);
////    d.fx = null;
////    d.fy = null;
////}
